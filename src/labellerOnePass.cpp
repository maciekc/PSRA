...
labellerOnePass::labellerOnePass(int XX, int YY)
{
	...
	mp_objects = new objectPL[MAX_LABELS];
}
...
//! Krok algorytmu
// -------------------------------------------------------------------------------------------------------------------
void labellerOnePass::step(const u8 *binaryImage, const u8 *moMask) {
	...
	for (int k = 0; k < MAX_LABELS; k++) {
		...
		mp_objects[k].noOfMovingPixels = 0;
	}


    // loop along rows (1 pixel boarder)
	for (unsigned int j = 1; j < mui_YY - 1; j++) {

		// label reset (afeter each row)
		lABCD[0] = 0;
		lABCD[1] = 0;
		lABCD[2] = 0;
		lABCD[3] = 0;
        ...
        // loop along columns (1 pixel boarder)
		for (unsigned int i = 1; i < mui_XX - 1; i++) {
            ...
            // TODO to explain ?
			// labels taken from buffers (HW simulation)
			lABCD[0] = lABCD[1]; //
			lABCD[1] = lABCD[2]; //
			...
			if (mX > 0) {
				...
				if ((mABCD[0] == 0) && (mABCD[1] == 0) && (mABCD[2] == 0) && (mABCD[3] == 0)) {      // newLabel (all pixels in context are background)
#ifdef DEBUG
                ...
                if (moMask[j*mui_XX + i] > 0)
                    mp_objects[mpui_labelLineBuffer[i]].noOfMovingPixels++;
    ...
    for (int i = 0; i < mui_currentLabel; i++) {
          int index = mpui_mergeTable[i];
            ...
            // TODO this requires correction
            if(index>0 && index < MAX_LABELS && index != i && mp_objects[index].m00 > 0) {
                ...
                mp_objects[index].noOfMovingPixels = mp_objects[index].noOfMovingPixels + mp_objects[i].noOfMovingPixels;
                ...
                mp_objects[i].m10 = 0;
                mp_objects[i].ymin = 0;
                mp_objects[i].ymax = 10000;
                mp_objects[i].xmin = 0;
                mp_objects[i].xmax = 10000;
                mp_objects[i].noOfMovingPixels =0;
...
// -------------------------------------------------------------------------------------------------------------------
//! Pobranie parametrow obiektow
// -------------------------------------------------------------------------------------------------------------------
void labellerOnePass::getParameters(unsigned int &noObjects, objectPL *objects) {
